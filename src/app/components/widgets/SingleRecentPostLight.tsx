...
const SingleRecentPostLight: React.FC<SingleRecentPostProps> = ({ blog }) => {
    ...
    return (
        <li>
            <div className="thumb">
                <Link to={`/blog-single-with-sidebar-light/${toBlogUrlFriendly(title2 || title || '')}`}>
            ...
            <div className="info">
                ...
                <Link to={`/blog-single-with-sidebar-light/${toBlogUrlFriendly(title2 || title || '')}`}>{truncatedTitle}</Link>
            </div>
        </li>
    );
};
